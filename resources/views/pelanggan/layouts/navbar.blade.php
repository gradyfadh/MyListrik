@php
    ...
    {
        return request()->routeIs($routeName) ? 'bg-yellow-500 text-white' : '';
    }
...
<nav class="bg-white shadow-sm py-4 px-20 sticky top-0 z-50">
    <div class="container mx-auto flex justify-between items-center px-4">
        <div class="flex items-center space-x-2">
            <span class="text-xl font-bold text-blue-500">MyListrik</span>
        </div>
        ...
        <ul class="flex space-x-6 text-sm text-gray-700">
            <li>
                <a href="{{ route('pelanggan.index') }}"
                    class="p-2 rounded-md font-medium hover:bg-yellow-500 hover:text-white transition duration-200 {{ activeNav('pelanggan.index') }}">
                    Beranda
                </a>
            </li>
            <li>
                <a href="{{ route('pelanggan.tagihan') }}"
                    class="p-2 rounded-md font-medium hover:bg-yellow-500 hover:text-white transition duration-200 {{ activeNav('pelanggan.tagihan') }}">
                    Tagihan
                </a>
            </li>
            <li class="relative group">
                <a
                    class="p-2 rounded-md font-medium hover:bg-yellow-500 hover:text-white transition duration-200 {{ request()->routeIs('riwayat-penggunaan') || request()->routeIs('riwayat-pembayaran') ? 'bg-yellow-500 text-white' : '' }}">
                    Riwayat
                </a>
                <ul
                    ...
                    <li>
                        <a href="{{ route('riwayat-penggunaan') }}"
                            class="block px-4 py-2 rounded hover:bg-yellow-300 {{ activeNav('riwayat-penggunaan') }}">
                            Riwayat Penggunaan
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('riwayat-pembayaran') }}"
                            class="block px-4 py-2 rounded hover:bg-yellow-300 {{ activeNav('riwayat-pembayaran') }}">
                            Riwayat Pembayaran
                        </a>
                    </li>
                </ul>
            ...
            <li>
                <a href="{{ route('tarif.listrik.pelanggan') }}"
                    class="p-2 rounded-md font-medium hover:bg-yellow-500 hover:text-white transition duration-200 {{ activeNav('tarif.listrik.pelanggan') }}">
                    Tarif Listrik
                </a>
            </li>
        </ul>
